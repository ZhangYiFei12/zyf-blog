...
**AI 文件阅读器**是一款跨平台桌面 AI 文档阅读软件,基于 **Electron + TypeScript + Vite** 构建。

它把"阅读文档"与"AI 理解文档"结合成一体:支持 **PDF / Word (DOCX) / EPUB / TXT / Markdown** 五种格式,可连接**本地大模型**(Ollama、LM Studio、vLLM 等)或**云端 API**(DeepSeek、Moonshot Kimi、OpenAI、智谱 GLM、阿里百炼等),自动生成**思维导图、大纲总结、全文翻译**,并支持**基于文档的多轮对话问答**与**一键导出**(Markdown / TXT / Word / PNG / XMind / PDF)。
...
| 解析器 | 格式 | 技术方案 |
|--------|------|----------|
| `pdf-parser.ts` | PDF | pdfjs-dist 提取文本层 + 章节页码 |
...
- **按需渲染**:打开 PDF 只解析文档结构(≈88ms),只渲染当前页,翻页按需取图并**预取相邻页**,几百页 PDF 也能秒开
- **动态分辨率**:基础倍率 2.0×,放大时当前页自动升级到 4.0× / 6.0×,保证 1:1 像素清晰(上限 6.0× 防内存爆炸)
- **LRU 缓存**:最多缓存 15 页,翻页 40 页也只驻留最近 15 页
- **FontFace polyfill**:用 `@napi-rs/canvas` 的 `GlobalFonts.register` 注册内嵌字体字节,解决 Electron 主进程无 DOM FontFace API 导致的中文 PDF 白页问题(修复后文字密度 0% → 2.77%)
...
- **并行处理**:多块**并发 3 路**同时请求,大幅缩短等待时间(1.0.17 优化)
...
**Q: 大文档处理慢?**
- 超过上下文窗口会自动分块,每块独立调用 AI 再合并。1.0.17 起多块**并行处理**,速度大幅提升

**Q: 如何导出思维导图图片?**
- 思维导图页**右键点击画布**导出 PNG;或导出 Markdown / XMind
