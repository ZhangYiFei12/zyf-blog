 ...
 * ≤100MB → files/<name> 仓库文件，浏览器后台上传（GitHub API commit），下载走本站 CDN
 * >100MB → GitHub Release 资产（≤2GB），由本机脚本 tools/publish-release.mjs 发布，
 *           下载链接指向 Release 资产
 * 元数据统一存 data/downloads.json（GitHub commit）。
 * 记录字段：id / name(原始文件名) / store: "repo" | "release" / url / filename(展示名)
 *           / size / version / desc / category / date / release?(release 用) / asset?(release 用)
 */

function dlSanitize(name) {
  // 文件名净化：保留中文/字母数字/._-，去除路径分隔符与危险字符
  return String(name || "").replace(/[\/\\\0]/g, "").replace(/\.\./g, "").trim();
}
...
function normFileMeta(input) {
  const f = {
    id: String(input.id || "").trim() || `f-${Date.now().toString(36)}`,
    name: dlSanitize(input.name || input.filename || ""),
    store: input.store === "release" ? "release" : "repo",
    url: String(input.url || "").trim(),
    filename: String(input.filename || input.name || "").trim() || "未命名",
  ...
  if (input.release) f.release = String(input.release).trim();
  if (input.assetId !== undefined && input.assetId !== null) f.assetId = String(input.assetId).trim();
  return f;
}
...
export async function onRequest(context) {
  ...
  // ---- POST /api/admin/files/repo（≤100MB 文件：浏览器上传入库 files/）----
  if (method === "POST" && rest.length === 2 && rest[0] === "files" && rest[1] === "repo") {
    const input = await request.json().catch(() => null);
    const name = dlSanitize(input && input.name);
    const b64 = input && input.data ? String(input.data) : "";
    if (!name) return json({ error: "缺少文件名" }, 400);
    // base64 膨胀 ~4/3；GitHub 单文件硬限 100MB，留安全裕量到 75MB 源文件
    const srcBytes = Math.floor(b64.length * 3 / 4);
    if (!b64) return json({ error: "文件内容为空" }, 400);
    if (srcBytes > 100 * 1024 * 1024) return json({ error: "文件超过 100MB，请使用 GitHub Release 方式（见 README）" }, 400);

    const files = await getDownloads(env);
    // 同名旧记录清理（避免残留）
    const existing = files.filter(f => f.name === name);
    const changes = [];
    if (existing.length) {
      for (const ex of existing) {
        if (ex.store === "repo" && ex.name) changes.push({ path: `files/${ex.name}`, delete: true });
      }
    }
    const remain = files.filter(f => f.name !== name);

    const entry = normFileMeta({
      name,
      store: "repo",
      filename: input.filename || name,
      size: Number(input.size) || srcBytes,
      version: input.version,
      desc: input.desc,
      category: input.category || "软件",
      url: `files/${name}`,
    });
    remain.unshift(entry);
    changes.push({ path: `files/${name}`, content: b64, encoding: "base64" });
    changes.push({ path: "data/downloads.json", content: JSON.stringify(remain, null, 2) + "\n" });

    const commitSha = await commitFiles(env, `📦 后台上传下载文件：${name}`, changes);
    return json({ ok: true, id: entry.id, commitSha, message: "已上传并提交，下载页将在部署后更新（约 1 分钟）" });
  }

  // ---- POST /api/admin/files/record（登记发布记录：本机脚本发布 Release 后调用）----
  if (method === "POST" && rest.length === 2 && rest[0] === "files" && rest[1] === "record") {
    const input = await request.json().catch(() => null);
    const files = await getDownloads(env);
    const entry = normFileMeta(input);
    if (!entry.name && !entry.url) return json({ error: "缺少必要信息" }, 400);
    const idx = files.findIndex(f => f.id === entry.id);
    if (idx >= 0) files[idx] = entry; else files.unshift(entry);
    const commitSha = await commitFiles(env, `📦 后台${idx >= 0 ? "更新" : "登记"}下载文件：${entry.filename || entry.name}`, [
  ...
  // ---- DELETE /api/admin/files（删除下载：记录 + 仓库文件 / Release 资产）----
  if (method === "DELETE" && rest.length === 1 && rest[0] === "files") {
    ...
    if (target.store === "release") {
      // 删除 GitHub Release 资产（尽力而为）
      const assetId = target.assetId;
      if (assetId) {
        try {
          const res = await gh(env, `/repos/${repo(env)}/releases/assets/${encodeURIComponent(assetId)}`, { method: "DELETE" });
          if (!res.ok) note = `（Release 资产删除失败 ${res.status}）`;
        } catch (e) {
          note = `（Release 资产删除失败：${e.message || e}）`;
        }
      }
    } else {
      // 删除仓库文件 files/<name>
      const p = `files/${target.name}`;
      changes.push({ path: p, delete: true });
    }
    changes.push({ path: "data/downloads.json", content: JSON.stringify(remaining, null, 2) + "\n" });
    const commitSha = await commitFiles(env, `🗑️ 后台删除下载文件：${target.filename || target.name}`, changes);
    return json({ ok: true, id, commitSha, message: `已删除并提交${note}` });
  }
